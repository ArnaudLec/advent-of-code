...
class Day07 {
	public static long calcCalibrationStatus(String input) {
		...
		for (String line : lines) {
			...
			long[] numbers = Arrays.stream(line.substring(indexOfResultEnd + 2).split(" ")).mapToLong(Long::parseLong)
					.toArray();
			if (calc(expectedResult, numbers, 0, 0)) {
	...
	private static boolean calc(long expectedResult, long[] numbers, long previousCalcResult, int i) {
		...
		for (Operation op : Operation.values()) {
			if (calc(expectedResult, numbers, op.apply(previousCalcResult, numbers[i]), i + 1)) {
				return true;
			}
		}
		return false;
